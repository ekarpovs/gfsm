...
# API
  ...
  def first_state_name(self) -> str:
    return self._fsm_impl.get('first-state-name')

  @property
  def init_action(self):
    return self._fsm_impl.get('init-action')
...
# 
  ...
  def _set_runtime_environment(self) -> None:
    ...
    user_action_wrapper_path = self.get_value(self._config, 'user-action-wrapper-path')
    if len(user_action_wrapper_path) > 1:
      sys.path.append(user_action_wrapper_path)
    user_action_wrapper_name = self.get_value(self._config, 'user-action-wrapper-name')
  ...
  def build(self) -> None:
    ...
    print("FSM bulder. Build the fsm implementation from: {}".format(self._config['info']))
    # build events
    ...
    # get init action
    init_action = self._load_action(self.get_value(self._definition, 'init-action'))

    # Setup FSM implementation
    self._fsm_impl['init-action'] = init_action
    self._fsm_impl['first-state-name'] = self.get_value(self._definition, 'first-state')
    # self._fsm_impl['events'] = events
    # self._fsm_impl['states'] = states
    # self._fsm_impl['transitions'] = {}
    return
